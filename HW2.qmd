...
tm_shape(la_map_ineq) +
  tm_polygons(col = "grade", 
   ...
   tm_layout(main.title = "HOLC Districts in LA County")
```
...
```{r}
kable(sum_current_conditions_table, col.names = c("HOLC Grade", "Low Income Percentage", "PM2.5 Mean Percentile", "Life Expectancy Percentile"))
```

# Do the test that on the units for pm
